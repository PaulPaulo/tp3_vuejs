  ...
  <!-- <div class="detail">
    <div class="detail-view card">
      <div class="data card-body">
        <img class="image" v-bind:src="IMG_URL+detail.name+'.png'" />
        <h2 class="card-title">{{ detail.name }}</h2>
        <h3>Types</h3>

        <div class="types" v-for="detail in detail.types" :key="detail.slot">
            <div class="type">
          <span :class="detail.type.name">{{ detail.type.name}}</span>
        </div>
        </div>
          <h3>Talents</h3>
          <div class="abilities" v-for="detail in detail.abilities" :key="detail.slot">
            <span class="ability">{{ detail.ability.name}}</span>
          </div>
      </div>
      <div class="property">
        <div class="left bold">Taille</div>
        <div class="right"><span>{{ detail.height }} cm</span></div>
       </div>
       <div class="property">
        <div class="left bold">Poids</div>
        <div class="right"><span>{{ detail.weight }} kg</span></div>
       </div>
...
export default {
  created() {
    ...
    //  .then(response =>  (this.detail = response.data.results.url))
  },
  props: [
    'pokemonUrl',
    'pokeImg',
